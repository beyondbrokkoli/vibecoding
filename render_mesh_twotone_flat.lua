...
-- Dynamic Face-Color Swapping (NO FOG - Maximum Performance)
-- ========================================================================
        ...
        for id = start_id, end_id do
            ...
            for i = 0, tCount - 1 do
                ...
                if Vert_Valid[i1] and Vert_Valid[i2] and Vert_Valid[i3] then
                    ...
                    -- Default to the baked frontface color
                    local final_color = Tri_BakedColor[idx]

                    -- If cross is >= 0, we are looking at the INSIDE of the geometry
                    if cross >= 0 then
                        -- Swap to Purple!
                        final_color = bit.bor(0xFF000000, bit.lshift(255, 16), bit.lshift(0, 8), 170)
                    end

                    -- Direct rasterization, absolute minimum overhead
